        ...
        if self.config.get('calculate_pi', True):
            ...
            if self.pyiqa_available:
                try:
                    ...
                    print(f"  ⚠ PI (pyiqa) initialization failed: {e}, using built-in implementation")
                    from metrics.pi import PI
                    self.metric_calculators['pi'] = PI(device=self.device)
            else:
                from metrics.pi import PI
                self.metric_calculators['pi'] = PI(device=self.device)
            print(f"  ✓ PI initialized")
...
class SRTester:
    ...
    def __init__(self, config_path: str):
        ...
        print(f"  - GT folder: {self.gt_folder if self.has_gt else 'None (only no-reference metrics)'}")
        ...
        for lq_path, gt_path in tqdm(pairs, desc="Testing progress"):
            ...
            if gt_path is not None and gt_path.exists():
                ...
                if gt_img.shape[:2] != sr_img.shape[:2]:
                    print(f"  ⚠ Size mismatch: SR={sr_img.shape[:2]}, GT={gt_img.shape[:2]}, skipping full-reference metrics")
                    gt_img = None
...
def main():
    ...
    if not config['data'].get('lq_folder'):
        raise ValueError("LQ image folder path must be specified (via config file or --lq_folder argument)")
